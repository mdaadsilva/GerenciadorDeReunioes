...
namespace Reunioes.API.Controllers
{
    ...
    public class ReservasController : ControllerBase
    {
        ...
        public async Task<IActionResult> CriarReserva([FromBody] CriarReservaDto reservaDto)
        {
            ...
            var inicioLocal = reservaDto.Inicio.ToLocalTime();
            var fimLocal = reservaDto.Fim.ToLocalTime();

            if (inicioLocal.Hour < 8 || fimLocal.Hour > 19 || (fimLocal.Hour == 19 && fimLocal.Minute > 0))
            {
        ...
        public async Task<IActionResult> ReagendarReserva(int id, [FromBody] CriarReservaDto reagendamentoDto)
        {
            ...
            using (var transaction = _session.BeginTransaction())
            {
                ...
                var inicioLocal = reagendamentoDto.Inicio.ToLocalTime();
                var fimLocal = reagendamentoDto.Fim.ToLocalTime();
